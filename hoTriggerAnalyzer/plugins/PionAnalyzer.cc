...
#include "HoMuonTrigger/hoTriggerAnalyzer/interface/PionAnalyzer.h"
...
PionAnalyzer::PionAnalyzer(const edm::ParameterSet& iConfig){
...
void
PionAnalyzer::analyze(const edm::Event& iEvent,
...
void 
PionAnalyzer::beginRun(const edm::Run& iRun,
...
void 
PionAnalyzer::endRun(const edm::Run& iRun, const edm::EventSetup& evSetup)
{
...
void
PionAnalyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
 ...
 * than delta R < 1
 */
const reco::GenParticle* PionAnalyzer::getBestGenMatch(float eta, float phi){
 ...
 */
bool PionAnalyzer::processTriggerDecision(std::string algorithmName,const edm::Event& iEvent){
...
DEFINE_FWK_MODULE(PionAnalyzer);
