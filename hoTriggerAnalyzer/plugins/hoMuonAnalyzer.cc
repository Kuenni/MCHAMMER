...
hoMuonAnalyzer::hoMuonAnalyzer(const edm::ParameterSet& iConfig) {
	...
	if (!isData) {
		...
		_l1MuonGenMatchInput = iConfig.getParameter<edm::InputTag>(
				"l1MuonGenMatchSrc");

		assoc.useDefaultPropagator();
		edm::ParameterSet parameters = iConfig.getParameter<edm::ParameterSet>(
	...
	deltaR_L1MuonMatching = iConfig.getParameter<double>(
			"maxDeltaRL1MuonMatching");
...
void hoMuonAnalyzer::analyze(const edm::Event& iEvent,
...
//			std::cout << coutPrefix << "Found no muon in acceptance in this Event." << std::endl;
	...
	for (; hoRecoIt != hoRecoHits->end(); hoRecoIt++) {
		...
		if (hoRecoIt->energy() >= threshold) {
			histogramBuilder.fillTimeHistogram(hoRecoIt->time(),
					"hoRecHitsAboveThr");
			histogramBuilder.fillEtaPhiHistograms(ho_eta, ho_phi,
					"hoRecHitsAboveThr");
			int hoIEta = hoRecoIt->id().ieta();
			int hoIPhi = hoRecoIt->id().iphi();
			histogramBuilder.fillIEtaIPhiHistogram(hoIEta, hoIPhi,
					"hoRecHitsAboveThr");
			recHitAbThrCounter++;
		}
		histogramBuilder.fillCountHistogram(horeco_key);
		histogramBuilder.fillEnergyVsIEta(hoRecoIt->energy(),
				hoRecoIt->id().ieta(), horeco_key);
...
//	singleMu10Trig = processTriggerDecision(singleMu10TrigName,iEvent);
	...
	histogramBuilder.fillMultiplicityHistogram(l1Muons->size(),
			"L1MuonPresent");
	if (l1Muons->size() > 0) {
	...
	for (unsigned int i = 0; i < l1Muons->size(); i++) {
		...
		histogramBuilder.fillBxIdVsPt(bl1Muon->bx(), bl1Muon->pt(),
				"L1MuonPresent");
...
//		listL1MuonPt.push_back(bl1Muon->pt());
		...
		if (MuonHOAcceptance::inGeomAccept(l1Muon_eta, l1Muon_phi)
				&& !hoMatcher->isInChimney(l1Muon_eta, l1Muon_phi)) {
			histogramBuilder.fillCountHistogram("L1MuonInGA_L1Dir");
			if (bl1Muon->bx() == 0)
				histogramBuilder.fillCountHistogram("L1MuoninGaBx0");
		}
		const HORecHit* matchedRecHit = hoMatcher->matchByEMaxInGrid(l1Muon_eta,
				l1Muon_phi, 2);
		if (matchedRecHit) {
			double hoEta, hoPhi;
			...
			histogramBuilder.fillTimeHistogram(matchedRecHit->time(),
					"L1MuonPresentHoMatch");
			histogramBuilder.fillDeltaTimeHistogram(matchedRecHit->time(),
					bl1Muon->bx(), "L1MuonPresentHoMatch");
			histogramBuilder.fillBxIdHistogram(bl1Muon->bx(),
					"L1MuonPresentHoMatch");
			histogramBuilder.fillEnergyVsIEta(matchedRecHit->energy(),
					matchedRecHit->id().ieta(), l1MuonWithHoMatch_key);
			histogramBuilder.fillEtaPhiHistograms(hoEta, hoPhi,
					l1MuonWithHoMatch_key);
			histogramBuilder.fillDeltaEtaDeltaPhiHistograms(l1Muon_eta, hoEta,
					l1Muon_phi, hoPhi, l1MuonWithHoMatch_key);
			histogramBuilder.fillEnergyVsPosition(hoEta, hoPhi,
					matchedRecHit->energy(), l1MuonWithHoMatch_key);
			if (MuonHOAcceptance::inGeomAccept(l1Muon_eta,
					l1Muon_phi/*,deltaR_Max,deltaR_Max*/)
					&& !hoMatcher->isInChimney(l1Muon_eta, l1Muon_phi)) {
				histogramBuilder.fillCountHistogram(
						"L1MuonPresentHoMatchInAcc");
				histogramBuilder.fillBxIdHistogram(bl1Muon->bx(),
						"L1MuonPresentHoMatchInAcc");
				...
				if (matchedRecHit->energy() >= threshold) {
					histogramBuilder.fillCountHistogram(
							"L1MuonPresentHoMatchInAccThr");

				}

				if (MuonHOAcceptance::inNotDeadGeom(l1Muon_eta,
						l1Muon_phi/*,deltaR_Max,deltaR_Max*/)) {
					histogramBuilder.fillCountHistogram(
							"L1MuonPresentHoMatchInAccNotDead");
					histogramBuilder.fillBxIdHistogram(bl1Muon->bx(),
							"L1MuonPresentHoMatchInAccNotDead");
					//This one is filled for the sake of completeness. The SiPM regions are hard-coded in the class!!
					if (MuonHOAcceptance::inSiPMGeom(l1Muon_eta,
							l1Muon_phi/*,deltaR_Max,deltaR_Max*/)) {
						histogramBuilder.fillCountHistogram(
								"L1MuonPresentHoMatchInAccNotDeadInSipm");
					}
				}
			}
			/**
			...
			for (int i = 0; i < 200; i += 2) {
				if (bl1Muon->pt() >= i)
					histogramBuilder.fillTrigRateHistograms(i,
							"L1MuonWithHoNoThr");
			}
			if (!isData) {
				const reco::GenParticle* bestGenMatch = getBestGenMatch(
						bl1Muon->eta(), bl1Muon->phi());
				if (bestGenMatch) {
					countGenMatches++;
					fillEfficiencyHistograms(bl1Muon->pt(), bestGenMatch->pt(),
							"L1MuonHoReco");
				}
			}
			//###########################################################
			...
			if (matchedRecHit->energy() >= threshold) {
				...
				histogramBuilder.fillBxIdHistogram(bl1Muon->bx(),
						"L1MuonAboveThr");
				histogramBuilder.fillDeltaTimeHistogram(matchedRecHit->time(),
						bl1Muon->bx(), "L1MuonAboveThr");
				histogramBuilder.fillTimeHistogram(matchedRecHit->time(),
						"L1MuonAboveThr");
				histogramBuilder.fillBxIdVsPt(bl1Muon->bx(), bl1Muon->pt(),
						"L1MuonAboveThr");
				histogramBuilder.fillEnergyVsIEta(matchedRecHit->energy(),
						matchedRecHit->id().ieta(),
						"L1MuonWithHoMatchAboveThr");
				histogramBuilder.fillEtaPhiHistograms(hoEta, hoPhi,
						"L1MuonWithHoMatchAboveThr_HO");
				histogramBuilder.fillDeltaEtaDeltaPhiHistograms(l1Muon_eta,
						hoEta, l1Muon_phi, hoPhi, "L1MuonWithHoMatchAboveThr");
				histogramBuilder.fillL1MuonPtHistograms(bl1Muon->pt(),
						"L1MuonWithHoMatchAboveThr");
				histogramBuilder.fillEnergyVsPosition(hoEta, hoPhi,
						matchedRecHit->energy(), "L1MuonWithHoMatchAboveThr");

				TH2D* hist = new TH2D("hoEnergyVsTime",
						"HO Energy vs. Time;Time / ns;E_{Rec} / GeV", 201,
						-100.5, 100.5, 2100, -5.0, 100.0);
				histogramBuilder.fillCorrelationHistogram(matchedRecHit->time(),
						matchedRecHit->energy(), "hoEnergyVsTime", hist);
				delete hist;
				...
				//RPC
				case 1:
					histogramBuilder.fillDeltaTimeHistogram(
							matchedRecHit->time(), bl1Muon->bx(),
							"RpcHoAboveThr");
					break;
					//DT
				case 2:
				...
				if (MuonHOAcceptance::inGeomAccept(l1Muon_eta,
					...
					if (MuonHOAcceptance::inNotDeadGeom(l1Muon_eta,
							l1Muon_phi/*,deltaR_Max,deltaR_Max*/)) {
						histogramBuilder.fillTimeHistogram(
								matchedRecHit->time(),
								"L1MuonAboveThrInAccNotDead");
						histogramBuilder.fillDeltaTimeHistogram(
								matchedRecHit->time(), bl1Muon->bx(),
								"L1MuonAboveThrInAccNotDead");
						histogramBuilder.fillCountHistogram(
						...
						histogramBuilder.fillBxIdHistogram(bl1Muon->bx(),
								"L1MuonAboveThrInAccNotDead");
						histogramBuilder.fillTrigHistograms(
								caloGeo->present(matchedRecHit->id()),
								"caloGeoPresent_L1MuonHoMatchAboveThrFilt");
						histogramBuilder.fillEnergyHistograms(
								matchedRecHit->energy(),
								"L1MuonWithHoMatchAboveThrFilt");
						histogramBuilder.fillEtaPhiHistograms(hoEta, hoPhi,
								"L1MuonWithHoMatchAboveThrFilt_HO");
						histogramBuilder.fillDeltaEtaDeltaPhiHistograms(
								l1Muon_eta, hoEta, l1Muon_phi, hoPhi,
								"L1MuonWithHoMatchAboveThrFilt");
						histogramBuilder.fillL1MuonPtHistograms(bl1Muon->pt(),
								"L1MuonWithHoMatchAboveThrFilt");
						histogramBuilder.fillEnergyVsPosition(hoEta, hoPhi,
						...
						case 3:
							histogramBuilder.fillDeltaTimeHistogram(
									matchedRecHit->time(), bl1Muon->bx(),
									"RpcAndDtHoAboveThrFilt");
							break;
						default:
							break;
						}
						if (!isData) {
							const reco::GenParticle* bestGenMatch =
									getBestGenMatch(bl1Muon->eta(),
											bl1Muon->phi());
							if (bestGenMatch) {
								fillEfficiencyHistograms(bl1Muon->pt(),
										bestGenMatch->pt(),
										"L1MuonHoRecoAboveThrFilt");
							}
						}
					}
				}
				...
				histogramBuilder.fillL1MuonPtHistograms(bl1Muon->pt(),
						l1MuonWithHoMatch_key);
				histogramBuilder.fillEtaPhiGraph(bl1Muon->eta(), bl1Muon->phi(),
						"L1MuonWithHoMatchAboveThr_L1Mu");
				...
				if (!isData) {
					...
					if (bestGenMatch) {
						fillEfficiencyHistograms(bl1Muon->pt(),
								bestGenMatch->pt(), "L1MuonHoRecoAboveThr");
						histogramBuilder.fillBxIdVsPt(bl1Muon->bx(),
								bl1Muon->pt(), "L1MuonAboveThrGenMatch");
					}
					//Try to find a corresponding Gen Muon
					edm::RefToBase<l1extra::L1MuonParticle> l1MuonCandiateRef(
							l1MuonView, i);
					reco::GenParticleRef ref =
							(*l1MuonGenMatches)[l1MuonCandiateRef];

					if (ref.isNonnull()) {
						string l1MuonAndHoRecoAndGenref_key =
								"L1MuonandHORecowithMipMatchAndGenMatch";
						//Make the pseudo trig rate plot
						for (int i = 0; i < 200; i += 2) {
							if (bl1Muon->pt() >= i)
								histogramBuilder.fillTrigRateHistograms(i,
										l1MuonAndHoRecoAndGenref_key);
						}
						histogramBuilder.fillPdgIdHistogram(ref->pdgId(),
								l1MuonWithHoMatch_key);
					} else {
						histogramBuilder.fillPdgIdHistogram(0,
								l1MuonWithHoMatch_key);
					}
				}
			}						// E > thr.
	...
	if (!isData)
		histogramBuilder.fillMultiplicityHistogram(countGenMatches,
				"nL1WithGenMatch");
	//################################
	//################################
	...
	if (l1Muons->size() == 0) {
		...
		for (; hoRecoIt != hoRecoHits->end(); hoRecoIt++) {
			if (hoRecoIt->energy() >= threshold) {
				...
				histogramBuilder.fillEnergyHistograms(hoRecoIt->energy(),
						"NoL1");
				histogramBuilder.fillEtaPhiHistograms(ho_eta, ho_phi, "NoL1");
				histogramBuilder.fillEnergyVsPosition(ho_eta, ho_phi,
						hoRecoIt->energy(), "NoL1");
			}
		}
		histogramBuilder.fillMultiplicityHistogram(recHitAbThrNoL1Counter,
				"NoL1");

		if (!isData) {
			for (reco::GenParticleCollection::const_iterator genIt =
					truthParticles->begin(); genIt != truthParticles->end();
					genIt++) {
				 ...
				 * #############################################
				 */
				TrackDetMatchInfo * muMatch = getTrackDetMatchInfo(*genIt,
						iEvent, iSetup);
				...
				histogramBuilder.fillEtaPhiGraph(muMatchEta, muMatchPhi,
						"NoL1TdmiAny");
				if (MuonHOAcceptance::inGeomAccept(muMatchEta, muMatchPhi)
						&& MuonHOAcceptance::inNotDeadGeom(muMatchEta,
								muMatchPhi)
						&& !hoMatcher->isInChimney(muMatchEta, muMatchPhi)) {
					histogramBuilder.fillEtaPhiGraph(genEta, genPhi,
							"NoL1GenInGA");
					histogramBuilder.fillEtaPhiGraph(muMatchEta, muMatchPhi,
							"NoL1TdmiInGA");
				}
			...
			for (reco::GenParticleCollection::const_iterator genIt =
				...
				if (MuonHOAcceptance::inGeomAccept(muMatchEta, muMatchPhi)
					...
					std::vector<const HORecHit*> crossedHoRecHits =
							muMatch->crossedHORecHits;
					calculateGridMatchingEfficiency(muMatchEta, muMatchPhi,
							genIt->pt(), "tdmi");
				}
				delete muMatch;

				const l1extra::L1MuonParticle* l1Part =
						functionsHandler->getBestL1MuonMatch(muMatchEta,
								muMatchPhi);
				if (l1Part) {
					double deltaEta = muMatchEta - l1Part->eta();
					double deltaPhi = FilterPlugin::wrapCheck(muMatchPhi,
							l1Part->phi());
					histogramBuilder.fillGraph(deltaEta, deltaPhi,
							"deltaEtaDeltaPhiTdmiL1");
				}
				const HORecHit* matchedRecHit = hoMatcher->matchByEMaxInGrid(
						genEta, genPhi, 2);
				if (matchedRecHit) {
					double hoEta =
							caloGeo->getPosition(matchedRecHit->id()).eta();
					double hoPhi =
							caloGeo->getPosition(matchedRecHit->id()).phi();
					histogramBuilder.fillDeltaEtaDeltaPhiEnergyHistogram(genEta,
							hoEta, genPhi, hoPhi, matchedRecHit->energy(),
							"WithSingleMu");
					if (matchedRecHit->energy() >= threshold) {
						histogramBuilder.fillDeltaEtaDeltaPhiHistograms(genEta,
								hoEta, genPhi, hoPhi, "WithSingleMuAboveThr");
						if (MuonHOAcceptance::inGeomAccept(genEta,
								genPhi/*,deltaR_Max,deltaR_Max*/)
								&& !hoMatcher->isInChimney(genEta, genPhi)) {
							histogramBuilder.fillDeltaEtaDeltaPhiHistograms(
									genEta, hoEta, genPhi, hoPhi,
									"WithSingleMuGeomAcc");
							if (MuonHOAcceptance::inNotDeadGeom(genEta,
									genPhi/*,deltaR_Max,deltaR_Max*/)) {
								histogramBuilder.fillDeltaEtaDeltaPhiHistograms(
										genEta, hoEta, genPhi, hoPhi,
										"WithSingleMuNotDead");
	...
	if (!singleMuOpenTrig) {
			...
			for (reco::GenParticleCollection::const_iterator genIt =
				...
				if (l1Muons->size() > 0) {
					...
					histogramBuilder.fillEtaPhiGraph(muMatchEta, muMatchPhi,
							"NoTrgNoL1TdmiAny");
					if (MuonHOAcceptance::inGeomAccept(muMatchEta, muMatchPhi)
							&& !hoMatcher->isInChimney(muMatchEta,
									muMatchPhi)) {
						histogramBuilder.fillEtaPhiGraph(genEta, genPhi,
								"NoTrgNoL1GenInGA");
						histogramBuilder.fillEtaPhiGraph(muMatchEta, muMatchPhi,
				...
				if (MuonHOAcceptance::inGeomAccept(muMatchEta, muMatchPhi)
					...
					histogramBuilder.fillEnergyVsPosition(muMatchEta,
							muMatchPhi, muMatch->hoCrossedEnergy(),
							"NoTrgTdmiXedE");
					const HORecHit* matchedRecHit =
							hoMatcher->matchByEMaxInGrid(muMatchEta, muMatchPhi,
									2);
					//Where is the Rec hit in a delta R cone with the largest E?
					//Did we find any?
					if (matchedRecHit) {
						histogramBuilder.fillCountHistogram(
								"NoTrgTdmiInGAHoMatch");
						double hoEta =
								caloGeo->getPosition(matchedRecHit->id()).eta();
						double hoPhi =
								caloGeo->getPosition(matchedRecHit->id()).phi();
						histogramBuilder.fillDeltaEtaDeltaPhiHistograms(
								muMatchEta, hoEta, muMatchPhi, hoPhi,
								"NoTrgTdmi");
						//Apply energy cut on the matched RecHit
						if (matchedRecHit->energy() >= threshold) {
							histogramBuilder.fillCountHistogram(
									"NoTrgTdmiInGAHoAboveThr");
							histogramBuilder.fillDeltaEtaDeltaPhiHistograms(
									muMatchEta, hoEta, muMatchPhi, hoPhi,
									"NoTrgTdmiAboveThr");
							histogramBuilder.fillEtaPhiGraph(muMatchEta,
									muMatchPhi, "NoTrgTdmiAboveThr");
							histogramBuilder.fillEtaPhiPtHistogram(muMatchEta,
									muMatchPhi, genIt->pt(),
									"NoTrgTdmiAboveThr");
							histogramBuilder.fillEtaPhiGraph(hoEta, hoPhi,
									"NoTrgTdmiAboveThrHoCoords");
							histogramBuilder.fillDeltaEtaDeltaPhiHistograms(
									genEta, hoEta, genPhi, hoPhi,
									"NoTrgGenAboveThr");
							histogramBuilder.fillEnergyVsPosition(muMatchEta,
									muMatchPhi, muMatch->hoCrossedEnergy(),
									"NoTrgTdmiAboveThrXedE");
							histogramBuilder.fillEnergyVsPosition(hoEta, hoPhi,
									matchedRecHit->energy(),
									"NoTrgTdmiAboveThrHoE");
							if ((muMatchEta > -0.35 && muMatchEta < -0.185)
									|| (muMatchEta > 0.16 && muMatchEta < 0.3)) {
								if ((muMatchPhi > 0.7 && muMatchPhi < 1.36)
										|| (muMatchPhi > 1.2 && muMatchPhi < 1.9)) {
									ofstream myfile;
									myfile.open("eventNumbers.txt",
											std::ios::app);
									myfile << iEvent.id().event() << std::endl;
								}
							}
							...
							histogramBuilder.fillEnergyVsPosition(muMatchEta,
									muMatchPhi, muMatch->hoCrossedEnergy(),
									"NoTrgTdmiBelowThrXedE");
							histogramBuilder.fillEtaPhiGraph(muMatchEta,
									muMatchPhi, "NoTrgTdmiBelowThr");
							histogramBuilder.fillEtaPhiPtHistogram(muMatchEta,
									muMatchPhi, genIt->pt(),
									"NoTrgTdmiBelowThr");
						}
						...
						histogramBuilder.fillEtaPhiGraph(muMatchEta, muMatchPhi,
								"NoTrgHoMatchFail");
						histogramBuilder.fillEtaPhiPtHistogram(muMatchEta,
								muMatchPhi, genIt->pt(), "NoTrgHoMatchFail");
					}
				}					//<-- in GA
				else {
					histogramBuilder.fillEtaPhiGraph(muMatchEta, muMatchPhi,
							"NoTrgTdmiNotInGA");
					histogramBuilder.fillEtaPhiPtHistogram(muMatchEta,
							muMatchPhi, genIt->pt(), "NoTrgTdmiNotInGA");
				}
				delete muMatch;
			...
			for (reco::GenParticleCollection::const_iterator genIt =
				...
				if (MuonHOAcceptance::inGeomAccept(muMatchEta, muMatchPhi)
					...
					const l1extra::L1MuonParticle* l1Ref =
							functionsHandler->getBestL1MuonMatch(genEta,
									genPhi);
					if (l1Ref) {
						histogramBuilder.fillCountHistogram(
								"SMuTrgFoundL1Match");
						...
						fillEfficiencyHistograms(l1Ref->pt(), genIt->pt(),
								"SMuTrgL1AndGenMatch");
						const HORecHit* matchedRecHit =
								hoMatcher->matchByEMaxInGrid(l1Muon_eta,
										l1Muon_phi, 2);
						//Check whether an HO match could be found by the delta R method
						if (matchedRecHit) {
							histogramBuilder.fillCountHistogram(
									"SMuTrgL1AndFoundHoMatch");
							//inspect the data where HO was above the threshold
							if (matchedRecHit->energy() >= threshold) {
								histogramBuilder.fillCountHistogram(
										"SMuTrgL1AndHoAboveThr");
								fillEfficiencyHistograms(l1Ref->pt(),
										genIt->pt(),
										"SMuTrgL1AndGenMatchHoAboveThr");
							}
						}
					}	//<-- Found L1 ref
					else {
						const HORecHit* matchedRecHit =
								hoMatcher->matchByEMaxInGrid(muMatchEta,
										muMatchPhi, 2);
						//Check whether an HO match could be found by the delta R method
						if (matchedRecHit) {
							histogramBuilder.fillCountHistogram(
									"SMuTrgAndFoundHoMatch");
							//inspect the data where HO was above the threshold
							if (matchedRecHit->energy() >= threshold) {
								histogramBuilder.fillCountHistogram(
										"SMuTrgAndHoAboveThr");
		...
		for (unsigned int i = 0; i < l1Muons->size(); i++) {
			...
			if (MuonHOAcceptance::inGeomAccept(l1Muon_eta, l1Muon_phi)
				...
				GlobalPoint l1Direction(bl1Muon->p4().X(), bl1Muon->p4().Y(),
						bl1Muon->p4().Z());
 ...
 */
bool hoMuonAnalyzer::processTriggerDecision(std::string algorithmName,
	...
	if (iErrorCode == 0) {
		...
		cout << coutPrefix << "trigger " << algorithmName
				<< " does not exist in the L1 menu" << endl;
	} else {
...
void hoMuonAnalyzer::beginRun(const edm::Run& iRun,
		const edm::EventSetup& evSetup) {

	bool useL1EventSetup = true;
	bool useL1GtTriggerMenuLite = true;
	std::cout << coutPrefix << "getL1GtRunCache" << std::endl;
	std::cout << coutPrefix << "UseL1EventSetup: " << useL1EventSetup
			<< std::endl;
	std::cout << coutPrefix << "UseL1GtTriggerMenuLite: "
			<< useL1GtTriggerMenuLite << std::endl;
...
void hoMuonAnalyzer::endRun(const edm::Run& iRun,
		const edm::EventSetup& evSetup) {
	...
	if (debug) {
		std::cout << coutPrefix << "The list contains " << listL1MuonPt.size()
				<< "unique entries:";
...
void hoMuonAnalyzer::fillDescriptions(
...
const l1extra::L1MuonParticle* hoMuonAnalyzer::getMatchedL1Object(
		trigger::TriggerObject hltObject,
	...
	for (unsigned int i = 0; i < l1muons->size(); i++) {
		...
		l1Eta = l1muon->eta();
		l1Phi = l1muon->phi();
		if (FilterPlugin::isInsideDeltaR(hltEta, l1Eta, hltPhi, l1Phi,
				deltaR_Max))
			return l1muon;
	}
	return NULL;
...
void hoMuonAnalyzer::defineTriggersOfInterest() {
	...
	hltNamesOfInterest.insert(
			pair<string, string>(hltIsoMu24_key, "HLT_IsoMu24_v18"));
	hltFiltersOfInterest.insert(
			pair<string, edm::InputTag>(hltIsoMu24_key,
					edm::InputTag("hltL3crIsoL1sMu16L1f0L2f16QL3"
							"f24QL3crIsoRhoFiltered0p15", "", "HLT")));

	string hltMu5_key = "hltMu5";
	hltNamesOfInterest.insert(pair<string, string>(hltMu5_key, "HLT_Mu5_v21"));
	hltFiltersOfInterest.insert(
			pair<string, edm::InputTag>(hltMu5_key,
					edm::InputTag("hltL3fL1sMu3L3Filtered5", "", "HLT")));

	string l1SingleMuOpen_key = "hlt_l1SingleMuOpen";
	hltNamesOfInterest.insert(
			std::pair<std::string, std::string>(l1SingleMuOpen_key,
					"HLT_L1SingleMuOpen_v7"));
	hltFiltersOfInterest.insert(
			std::pair<std::string, edm::InputTag>(l1SingleMuOpen_key,
					edm::InputTag("hltL1MuOpenL1Filtered0", "", "HLT")));
 ...
 */
TrackDetMatchInfo* hoMuonAnalyzer::getTrackDetMatchInfo(
		reco::GenParticle genPart, const edm::Event& iEvent,
		const edm::EventSetup& iSetup) {
	//Create the Track det match info
	GlobalPoint vertexPoint(genPart.vertex().X(), genPart.vertex().Y(),
			genPart.vertex().Z());
	GlobalVector mom(genPart.momentum().x(), genPart.momentum().y(),
			genPart.momentum().z());
	int charge = genPart.charge();
	const FreeTrajectoryState *freetrajectorystate_ = new FreeTrajectoryState(
			vertexPoint, mom, charge, &(*theMagField));
	TrackDetMatchInfo* info = new TrackDetMatchInfo(
			assoc.associate(iEvent, iSetup, *freetrajectorystate_,
					assocParams));
 ...
 */
void hoMuonAnalyzer::fillEfficiencyHistograms(double ptMeasured, double ptReal,
		std::string key) {
...
void hoMuonAnalyzer::printChannelQualities(const edm::EventSetup& iSetup) {
	...
	TH2D* channelStatusHist = new TH2D("channelStatusHist",
			"Channel status of HO", 34, -16.5, 16.5, 73, -0.5, 73.5);
	for (ieta = -15; ieta <= 15; ieta++) {
		if (ieta != 0) {
			for (iphi = 1; iphi <= 72; iphi++) {
				HcalDetId did(HcalOuter, ieta, iphi, 4);
				const HcalChannelStatus *mystatus = myqual->getValues(
						did.rawId());
				channelStatusHist->SetBinContent(
						channelStatusHist->FindBin(ieta, iphi),
						mystatus->getValue());
 ...
 */
void hoMuonAnalyzer::analyzeNoSingleMuEventsL1Loop(const edm::Event& iEvent,
		const edm::EventSetup& iSetup) {
	for (unsigned int i = 0; i < l1Muons->size(); i++) {
		const l1extra::L1MuonParticle* l1Muon = &(l1Muons->at(i));
		edm::RefToBase<l1extra::L1MuonParticle> l1MuonCandiateRef(l1MuonView,
				i);
		reco::GenParticleRef ref = (*l1MuonGenMatches)[l1MuonCandiateRef];
		if (ref.isNonnull()) {
			histogramBuilder.fillEfficiency(true, l1Muon->pt(),
					"L1GenRefNoSingleMu");
			//Once there is a gen ref, get the Track det match info
			TrackDetMatchInfo * muMatch = getTrackDetMatchInfo(*ref, iEvent,
					iSetup);
			double muMatchEta = muMatch->trkGlobPosAtHO.eta();
			double muMatchPhi = muMatch->trkGlobPosAtHO.phi();
			delete muMatch;
			...
			if (MuonHOAcceptance::inGeomAccept(muMatchEta, muMatchPhi)
				...
				calculateGridMatchingEfficiency(&*l1Muon, ref->pt(),
						"L1GenRefNoSingleMuInGa");
			}

		} else {
			histogramBuilder.fillEfficiency(false, l1Muon->pt(),
					"L1GenRefNoSingleMu");
			histogramBuilder.fillEtaPhiGraph(l1Muon->eta(), l1Muon->phi(),
					"L1GenRefNoSingleMuFail");
		}
	}
}
 ...
 */
void hoMuonAnalyzer::analyzeNoSingleMuEventsGenLoop(const edm::Event& iEvent,
		const edm::EventSetup& iSetup) {
	for (reco::GenParticleCollection::const_iterator genIt =
			truthParticles->begin(); genIt != truthParticles->end(); genIt++) {

		histogramBuilder.fillPtHistogram(genIt->pt(), "NoSingleMu");

		//Once there is a gen ref, get the Track det match info
		TrackDetMatchInfo * muMatch = getTrackDetMatchInfo(*genIt, iEvent,
				iSetup);
		double muMatchEta = muMatch->trkGlobPosAtHO.eta();
		double muMatchPhi = muMatch->trkGlobPosAtHO.phi();
		delete muMatch;
		...
		if (MuonHOAcceptance::inGeomAccept(muMatchEta, muMatchPhi)
			...
			calculateGridMatchingEfficiency(genIt->eta(), genIt->phi(),
					genIt->pt(), "NoSingleMuInGa");
		}
	}
}
 ...
 */
void hoMuonAnalyzer::analyzeWithGenLoop(const edm::Event& iEvent,
		const edm::EventSetup& iSetup) {
	for (reco::GenParticleCollection::const_iterator genIt =
			truthParticles->begin(); genIt != truthParticles->end(); genIt++) {
		float genEta = genIt->eta();
		...
		if (l1Part) {
			 ...
			 * used
			 */
			histogramBuilder.fillCorrelationHistogram(genIt->pt(), l1Part->pt(),
					"L1MuonPtGenLoop");
			histogramBuilder.fillCountHistogram("GenAndL1Muon");
			calculateGridMatchingEfficiency(&*l1Part, l1Part->pt(),
					"L1MuonTruth");
			...
			matchedRecHit = hoMatcher->matchByEMaxInGrid(l1Part->eta(),
					l1Part->phi(), 2);
			if (matchedRecHit) {
				if (matchedRecHit->energy() > threshold) {
					fillEfficiencyHistograms(l1Part->pt(), genIt->pt(),
							"GenAndL1MuonAndHoAboveThr");
					histogramBuilder.fillEnergyVsIEta(matchedRecHit->energy(),
							matchedRecHit->id().ieta(), "l1TruthAndHoMatch");
					histogramBuilder.fillCountHistogram(
							"GenAndL1MuonAndHoAboveThr");
					double hoPhi = hoMatcher->getRecHitPhi(matchedRecHit);
					double hoIPhi = matchedRecHit->id().iphi();
					histogramBuilder.fillCorrelationGraph(hoPhi, l1Part->phi(),
							"l1PhiVsHoPhi");
					histogramBuilder.fillCorrelationGraph(hoIPhi, l1Part->phi(),
							"l1PhiVsHoIPhi");
					histogramBuilder.fillCorrelationGraph(hoIPhi, hoPhi,
							"hoPhiVsHoIPhi");

					TH2D* hist = new TH2D("hoTruthEnergyVsTime",
							"HO Energy vs. Time;Time / ns;E_{Rec} / GeV", 201,
							-100.5, 100.5, 2100, -5.0, 100.0);
					histogramBuilder.fillCorrelationHistogram(
							matchedRecHit->time(), matchedRecHit->energy(),
							"hoTruthEnergyVsTime", hist);
					delete hist;

					//Implement efficiency analysis for time window
					if (matchedRecHit->time() > -12.5
							&& matchedRecHit->time() < 12.5) {
 ...
 */
void hoMuonAnalyzer::fillAverageEnergyAroundL1Direction(
		const l1extra::L1MuonParticle* l1Muon, std::string key) {
	int gridSize = 5;
	for (auto recHitIt = hoRecoHits->begin(); recHitIt != hoRecoHits->end();
			recHitIt++) {
		if (hoMatcher->isRecHitInGrid(l1Muon->eta(), l1Muon->phi(), &*recHitIt,
				gridSize)) {
			double hoEta = hoMatcher->getRecHitEta(&*recHitIt);
			double hoPhi = hoMatcher->getRecHitPhi(&*recHitIt);

			histogramBuilder.fillAverageEnergyHistograms(l1Muon->eta(), hoEta,
					l1Muon->phi(), hoPhi, recHitIt->energy(),
					"averageEnergyAroundPoint" + key);
			histogramBuilder.fillDeltaEtaDeltaPhiEnergyHistogram(l1Muon->eta(),
					hoEta, l1Muon->phi(), hoPhi, recHitIt->energy(),
					"averageEnergyAroundPoint" + key); //Use this function for the 1D distributions for each delta eta and delta phi

			double deltaPhi;
			deltaPhi = FilterPlugin::wrapCheck(l1Muon->phi(),
					hoMatcher->getRecHitPhi(&*recHitIt));
			...
			TH1D* hist1D = new TH1D(("deltaPhi" + key).c_str(),
					"#Delta#phi;#Delta#phi;N Entries", 81,
					-40 * HoMatcher::HALF_HO_BIN / 2.
							- HoMatcher::HALF_HO_BIN / 4.,
					40 * HoMatcher::HALF_HO_BIN / 2.
							+ HoMatcher::HALF_HO_BIN / 4.);
			histogramBuilder.fillHistogram(deltaPhi, "deltaPhi" + key, hist1D);
			delete hist1D;

			TH1D* histL1Phi = new TH1D(("averageEnergyL1Phi" + key).c_str(),
					"L1 #phi;#phi;N Entries", 145,
					-36 * HoMatcher::HO_BIN - HoMatcher::HALF_HO_BIN / 2.,
					36 * HoMatcher::HO_BIN + HoMatcher::HALF_HO_BIN / 2.);
			histogramBuilder.fillHistogram(l1Muon->phi(),
					"averageEnergyL1Phi" + key, histL1Phi);
			delete histL1Phi;

			TH1D* histHoPhi = new TH1D(("averageEnergyHoPhi" + key).c_str(),
					"HO #phi;#phi;N Entries", 145,
					-36 * HoMatcher::HO_BIN - HoMatcher::HALF_HO_BIN / 2.,
					36 * HoMatcher::HO_BIN + HoMatcher::HALF_HO_BIN / 2.);
			histogramBuilder.fillHistogram(hoPhi, "averageEnergyHoPhi" + key,
					histHoPhi);
			delete histHoPhi;

			double variableBinArray[] = { 0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4,
					4.5, 5, 6, 7, 8, 10, 12, 14, 16, 18, 20, 25, 30, 35, 40, 45,
					50, 60, 70, 80, 100, 120, 140, 200 };

			TH2D* hist = new TH2D(("shiftCheckDeltaPhiVsL1Pt" + key).c_str(),
					"#Delta#phi shift check;p_{T} / GeV;#Delta#phi", 32,
					variableBinArray, 73,
					-36 * HoMatcher::HO_BIN - HoMatcher::HALF_HO_BIN,
					36 * HoMatcher::HO_BIN + HoMatcher::HALF_HO_BIN);

			histogramBuilder.fillCorrelationHistogram(l1Muon->pt(), deltaPhi,
					"shiftCheckDeltaPhiVsL1Pt" + key, hist);
			delete hist;
			...
			hist = new TH2D(("shiftCheckDeltaPhiVsPhi" + key).c_str(),
					"#Delta#phi shift check;#phi;#Delta#phi", 289,
					-36 * HoMatcher::HO_BIN - HoMatcher::HALF_HO_BIN / 4.,
					36 * HoMatcher::HO_BIN + HoMatcher::HALF_HO_BIN / 4., 145,
					-36 * HoMatcher::HO_BIN - HoMatcher::HALF_HO_BIN / 2.,
					36 * HoMatcher::HO_BIN + HoMatcher::HALF_HO_BIN / 2.);
			histogramBuilder.fillCorrelationHistogram(l1Muon->phi(), deltaPhi,
					"shiftCheckDeltaPhiVsPhi" + key, hist);
			delete hist;

			histogramBuilder.fillCorrelationGraph(l1Muon->phi(), deltaPhi,
					"shiftCheckDeltaPhiVsPhiGraph" + key);
...
//			delete hist;
			...
			//Delta phi vs l1 eta
			hist = new TH2D(("shiftCheckDeltaPhiVsL1Eta" + key).c_str(),
					"#Delta#phi shift check;#eta_{L1};#Delta#phi", 30,
					-15 * 0.1, 15 * 0.1,			//L1 has 0.1 bins in eta
					73, -36 * HoMatcher::HO_BIN - HoMatcher::HALF_HO_BIN,
					36 * HoMatcher::HO_BIN + HoMatcher::HALF_HO_BIN);
			histogramBuilder.fillCorrelationHistogram(l1Muon->eta(), deltaPhi,
					"shiftCheckDeltaPhiVsL1Eta" + key, hist);
			delete hist;
...
//			delete hist;
	...
	const HORecHit* matchedRecHit = 0;
	matchedRecHit = hoMatcher->matchByEMaxInGrid(l1Muon->eta(), l1Muon->phi(),
			5);
	if (matchedRecHit) {
		histogramBuilder.fillDeltaEtaDeltaPhiHistogramsWithWeights(
				l1Muon->eta(), float(hoMatcher->getRecHitEta(matchedRecHit)),
				l1Muon->phi(), float(hoMatcher->getRecHitPhi(matchedRecHit)),
				matchedRecHit->energy(),
				("averageEMaxAroundPoint" + key).c_str());
		histogramBuilder.fillDeltaEtaDeltaPhiEnergyHistogram(l1Muon->eta(),
				float(hoMatcher->getRecHitEta(matchedRecHit)), l1Muon->phi(),
				float(hoMatcher->getRecHitPhi(matchedRecHit)),
				matchedRecHit->energy(),
				("averageEMaxAroundPoint" + key).c_str());
	}
}
...
void hoMuonAnalyzer::fillHoGeomAcceptanceGraph(reco::GenParticle genPart) {
	if (MuonHOAcceptance::inGeomAccept(genPart.eta(), genPart.phi())
		...
		histogramBuilder.fillEtaPhiGraph(genPart.eta(), genPart.phi(),
				"HoGeomAcceptance");
	}
}
 ...
 */
void hoMuonAnalyzer::fillGridMatchingQualityCodes(
		const l1extra::L1MuonParticle* l1muon, float truePt, std::string key) {
	GlobalPoint direction(l1muon->p4().X(), l1muon->p4().Y(), l1muon->p4().Z());
	//#####
	// Central tile
	//#####
	double variableBinArray[] = { 0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5, 6,
			7, 8, 9, 10, 12, 14, 16, 18, 20, 25, 30, 35, 40, 45, 50, 60, 70, 80,
			100, 120, 140, 180 };
	int l1MuonQuality = l1muon->gmtMuonCand().quality();
	histogramBuilder.fillMultiplicityHistogram(l1MuonQuality,
			key + "AllQualityCodes");
	if (hoMatcher->hasHoHitInGrid(direction, 0)) {
		histogramBuilder.fillMultiplicityHistogram(l1MuonQuality,
				key + "QualityCodesCentral");
		fillEfficiencyHistograms(l1muon->pt(), truePt, key + "GenPtCentral");
	} else {
		histogramBuilder.fillMultiplicityHistogram(l1MuonQuality,
				key + "QualityCodesCentralFail");
		TH2D* hist = new TH2D((key + "pTvsQCCentralFail").c_str(),
				"p_{T} vs. QC (Central);QC;p_{T} / Gev", 7, 1.5, 8.5, 33,
				variableBinArray);
		histogramBuilder.fillCorrelationHistogram(l1MuonQuality, l1muon->pt(),
				key + "pTvsQCCentralFail", hist);
		delete hist;
	}
	//#####
	// 3 x 3
	...
	if (hoMatcher->hasHoHitInGrid(direction, 1)) {
		...
		histogramBuilder.fillMultiplicityHistogram(l1MuonQuality,
				key + "QualityCodes3x3Fail");
		TH2D* hist = new TH2D((key + "pTvsQC3x3Fail").c_str(),
				"p_{T} vs. QC (3x3);QC;p_{T} / Gev", 7, 1.5, 8.5, 33,
				variableBinArray);
		histogramBuilder.fillCorrelationHistogram(l1MuonQuality, l1muon->pt(),
				key + "pTvsQC3x3Fail", hist);
		delete hist;
	}
	//#####
	// 5 x 5
	...
	if (hoMatcher->hasHoHitInGrid(direction, 2)) {
		...
		histogramBuilder.fillMultiplicityHistogram(l1MuonQuality,
				key + "QualityCodes5x5Fail");
		TH2D* hist = new TH2D((key + "pTvsQC5x5Fail").c_str(),
				"p_{T} vs. QC (5x5);QC;p_{T} / Gev", 7, 1.5, 8.5, 33,
				variableBinArray);
		histogramBuilder.fillCorrelationHistogram(l1MuonQuality, l1muon->pt(),
				key + "pTvsQC5x5Fail", hist);
		delete hist;
	}
 ...
 */
void hoMuonAnalyzer::calculateGridMatchingEfficiency(
		const l1extra::L1MuonParticle* l1muon, float pt, std::string key) {
	...
	const HORecHit* recHit = hoMatcher->getClosestRecHitInGrid(l1muon->eta(),
			l1muon->phi(), 2);
	for (int i = 0; i < 3; i++) {
		std::string gridString = CommonFunctionsHandler::getGridString(i);
		if (!recHit) {
			histogramBuilder.fillBxIdVsPt(l1muon->bx(), l1muon->pt(),
					key + gridString + "Fail");
		} else {
			int qualityCode = 0;
			...
			if (hoMatcher->isRecHitInGrid(l1muon->eta(), l1muon->phi(), recHit,
					i)) {
				histogramBuilder.fillBxIdVsPt(l1muon->bx(), l1muon->pt(),
						key + gridString + "Match");
				histogramBuilder.fillQualityCodeVsPt(qualityCode, l1muon->pt(),
						key + gridString + "Match");
				...
				histogramBuilder.fillEtaPhiHistograms(
						hoMatcher->getRecHitEta(recHit),
						hoMatcher->getRecHitPhi(recHit), key + gridString);
			} else {
				histogramBuilder.fillBxIdVsPt(l1muon->bx(), l1muon->pt(),
						key + gridString + "Fail");
				histogramBuilder.fillQualityCodeVsPt(qualityCode, l1muon->pt(),
...
#define L1BXID_M_3		0000001xxxx....
	 ...
	 */
	for (auto l1MuonIt = l1Muons->begin(); l1MuonIt != l1Muons->end();
			l1MuonIt++) {
 ...
 */
void hoMuonAnalyzer::calculateGridMatchingEfficiency(double eta, double phi,
		float pt, std::string key) {
	const HORecHit* recHit = hoMatcher->getClosestRecHitInGrid(eta, phi, 2);
	for (int i = 0; i < 3; i++) {
		if (!recHit) {
			fillGridMatchingHistograms(false, i, pt, 999, key, eta, phi);
		} else {
			fillGridMatchingHistograms(
					hoMatcher->isRecHitInGrid(eta, phi, recHit, i), i, pt,
					recHit->time(), key, eta, phi);
		}
	}
}
 ...
 */
void hoMuonAnalyzer::fillGridMatchingHistograms(bool passed, int grid,
		double pt, double time, std::string key, double eta, double phi) {
	...
	if (passed && isInTimeWindow(time)) {
		histogramBuilder.fillEfficiency(true, pt,
				key + "TimeWindow" + gridString);
	} else {
		histogramBuilder.fillEfficiency(false, pt,
				key + "TimeWindow" + gridString);
	}
}
...
void hoMuonAnalyzer::analyzeL1Resolution() {
	for (auto patMuonIt = patMuons->begin(); patMuonIt != patMuons->end();
			patMuonIt++) {
		const l1extra::L1MuonParticle* l1Part = 0;
		l1Part = functionsHandler->getBestL1MuonMatch(patMuonIt->eta(),
				patMuonIt->phi());
		if (l1Part) {
			...
			histogramBuilder.fillL1ResolutionHistogram(l1Part->pt(),
					patMuonIt->pt(), "L1MuonTruth");
			if (isTight) {
				histogramBuilder.fillL1ResolutionHistogram(l1Part->pt(),
						patMuonIt->pt(), "L1MuonTightTruth");
				const HORecHit* matchedRecHit = 0;
				matchedRecHit = hoMatcher->matchByEMaxInGrid(l1Part->eta(),
						l1Part->phi(), 2);
				if (matchedRecHit) {
					histogramBuilder.fillL1ResolutionHistogram(l1Part->pt(),
							patMuonIt->pt(), "L1MuonTightTruthHoMatch");
				} else {
					histogramBuilder.fillL1ResolutionHistogram(l1Part->pt(),
							patMuonIt->pt(), "L1MuonTightTruthNotHoMatch");
				}
			}
			const HORecHit* matchedRecHit = 0;
			matchedRecHit = hoMatcher->matchByEMaxInGrid(l1Part->eta(),
			...
			if (matchedRecHit) {
				histogramBuilder.fillL1ResolutionHistogram(l1Part->pt(),
						patMuonIt->pt(), "L1MuonTruthHoMatch");
				if (isTight) {
					histogramBuilder.fillL1ResolutionHistogram(l1Part->pt(),
							patMuonIt->pt(), "L1MuonTruthHoMatchTight");
				} else {
					histogramBuilder.fillL1ResolutionHistogram(l1Part->pt(),
							patMuonIt->pt(), "L1MuonTruthHoMatchNotTight");
				}
			} else {
				histogramBuilder.fillL1ResolutionHistogram(l1Part->pt(),
						patMuonIt->pt(), "L1MuonTruthNotHoMatch");
			}
		}
	}
}
...
void hoMuonAnalyzer::recoControlPlots() {
	for (auto recoIt = recoMuons->begin(); recoIt != recoMuons->end();
			recoIt++) {
		histogramBuilder.fillPtHistogram(recoIt->pt(), "recoMuons");
		histogramBuilder.fillEtaPhiGraph(recoIt->eta(), recoIt->phi(),
				"recoMuons");
		histogramBuilder.fillEtaPhiHistograms(recoIt->eta(), recoIt->phi(),
				"recoMuons");
	}
	for (auto patMuonIt = patMuons->begin(); patMuonIt != patMuons->end();
			++patMuonIt) {
		histogramBuilder.fillPtHistogram(patMuonIt->pt(), "patMuons");
		histogramBuilder.fillEtaPhiGraph(patMuonIt->eta(), patMuonIt->phi(),
				"patMuons");
		histogramBuilder.fillEtaPhiHistograms(patMuonIt->eta(),
				patMuonIt->phi(), "patMuons");
		if (patMuonIt->isTightMuon(getPrimaryVertex())) {
			histogramBuilder.fillPtHistogram(patMuonIt->pt(), "patMuonsTight");
			histogramBuilder.fillEtaPhiGraph(patMuonIt->eta(), patMuonIt->phi(),
					"patMuonsTight");
			histogramBuilder.fillEtaPhiHistograms(patMuonIt->eta(),
					patMuonIt->phi(), "patMuonsTight");
			const HORecHit* recHit = hoMatcher->matchByEMaxInGrid(
					patMuonIt->eta(), patMuonIt->phi(), 2);
			if (recHit) {
				histogramBuilder.fillEtaPhiHistograms(
						hoMatcher->getRecHitEta(recHit),
						hoMatcher->getRecHitPhi(recHit), "patMuonsTight_HO");
				histogramBuilder.fillEnergyVsIEta(recHit->energy(),
						recHit->id().ieta(), "patMuonsTight");
			}
		}
	}
}
	...
	for (auto patMuonIt = patMuons->begin(); patMuonIt != patMuons->end();
		...
		if (patMuonIt->isTightMuon(getPrimaryVertex())) {
			...
			if (l1Part) {
				...
				histogramBuilder.fillEtaPhiGraph(l1Part->eta(), l1Part->phi(),
						"L1TightMuons");
 ...
 */
void hoMuonAnalyzer::processRecoInformation(const edm::Event& iEvent,
		const edm::EventSetup& iSetup) {
	analyzeL1Resolution();
	recoControlPlots();
...
void hoMuonAnalyzer::processGenInformation(const edm::Event& iEvent,
		const edm::EventSetup& iSetup) {
	//Some Gen stuff
	...
	for (reco::GenParticleCollection::const_iterator genIt =
		...
		if (abs(genIt->pdgId()) == 13) {
			...
			histogramBuilder.fillEtaPhiGraph(genIt->eta(), genIt->phi(),
					gen_key);
	...
	int failedMatches = 0;
	//Define iterators
	l1extra::L1MuonParticleCollection::const_iterator bl1Muon =
			l1Muons->begin();
	l1extra::L1MuonParticleCollection::const_iterator el1Muon = l1Muons->end();
	for (unsigned int i = 0; i < l1Muons->size(); i++) {
		const l1extra::L1MuonParticle* bl1Muon = &(l1Muons->at(i));
		const reco::GenParticle* genMatch = getBestGenMatch(bl1Muon->eta(),
				bl1Muon->phi());
		if (genMatch) {
			successfulMatches++;
			histogramBuilder.fillDeltaVzHistogam(
					(genMatch->vz() - bl1Muon->vz()), l1muon_key);
			histogramBuilder.fillCorrelationHistogram(genMatch->pt(),
					bl1Muon->pt(), "L1MuonPt");
			histogramBuilder.fillEtaPhiGraph(genMatch->eta(), genMatch->phi(),
					"L1ToGen");
			histogramBuilder.fillEtaPhiPtHistogram(genMatch->eta(),
					genMatch->phi(), genMatch->pt(), "L1ToGen");
			...
			if (bl1Muon->bx() != 0) {
				...
				histogramBuilder.fillEtaPhiGraph(genMatch->eta(),
						genMatch->phi(), "BxWrongGen");
				histogramBuilder.fillEtaPhiPtHistogram(genMatch->eta(),
						genMatch->phi(), genMatch->pt(), "BxWrongGen");
				/**
				 ...
				 */
				histogramBuilder.fillMultiplicityHistogram(
						bl1Muon->gmtMuonCand().detector(),
						"detectorIndexBxWrong");
			} else {
				histogramBuilder.fillPtHistogram(genMatch->pt(), "BxRightGen");
				histogramBuilder.fillEtaPhiGraph(genMatch->eta(),
						genMatch->phi(), "BxRightGen");
				histogramBuilder.fillEtaPhiPtHistogram(genMatch->eta(),
						genMatch->phi(), genMatch->pt(), "BxRightGen");
				histogramBuilder.fillMultiplicityHistogram(
						bl1Muon->gmtMuonCand().detector(),
						"detectorIndexBxRight");
			}
			 ...
			 */
			const HORecHit* matchedRecHit = hoMatcher->matchByEMaxInGrid(
					bl1Muon->eta(), bl1Muon->phi(), 2);
			if (matchedRecHit) {
				if (matchedRecHit->energy() > threshold)
					fillEfficiencyHistograms(bl1Muon->pt(), genMatch->pt(),
							"L1MuonAndHoAboveThr");
			}
		} else {
			failedMatches++;
	 ...
	 */
	histogramBuilder.fillMultiplicityHistogram(failedMatches,
			"failedL1ToGenMatches");
	histogramBuilder.fillMultiplicityHistogram(successfulMatches,
			"successfulL1ToGenMatches");
...
const reco::Vertex hoMuonAnalyzer::getPrimaryVertex() {
	...
	if (vertexColl.isValid()) {
		for (unsigned int ind = 0; ind < vertexColl->size(); ++ind) {
			if ((*vertexColl)[ind].isValid()
					&& !((*vertexColl)[ind].isFake())) {
...
void hoMuonAnalyzer::makeHoRecHitThresholdScan() {
	...
	for (double eThr = 0.025; eThr <= 5; eThr += 0.025) {
		...
		histogramBuilder.fillMultiplicityHistogram(thrCounter,
				Form("recHitThrScan%d", iterationCounter));
		iterationCounter++;
	}
}
 ...
 */
void hoMuonAnalyzer::analyzeL1MuonsForGhosts(const edm::Event& iEvent,
		const edm::EventSetup& iSetup) {
//	/**
